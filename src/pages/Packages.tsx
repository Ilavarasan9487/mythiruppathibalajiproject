      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="text-center bg-white p-12 rounded-3xl shadow-sm max-w-2xl mx-auto">
            <p className="text-gray-500 text-lg font-medium">No packages are currently listed.</p>
          </div>
        ) : (
              ...
              <div key={pkg.id} className="bg-white rounded-3xl overflow-hidden shadow-lg flex flex-col border border-gray-100 hover:shadow-xl transition-shadow">
                ...
                <div className="p-8 flex-grow flex flex-col">
                  <div className="flex justify-between items-start mb-4 gap-4">
                    <h3 className="text-2xl font-bold text-brand-blue text-balance leading-snug">{pkg.title}</h3>
                    <div className="flex items-center bg-brand-gold/10 px-3 py-1.5 rounded-lg text-brand-gold text-sm font-bold flex-shrink-0">
                      <Star className="w-4 h-4 mr-1.5 fill-current" /> 4.9
                    </div>
                  </div>
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between">
                    <div>
                      <span className="text-sm text-gray-500 font-medium block mb-1">Starting from</span>
                      <span className="text-3xl font-bold text-brand-blue">₹{pkg.price}</span>
                    </div>
                    <button className="bg-brand-gold text-brand-blue px-6 py-3 rounded-xl font-bold hover:bg-yellow-400 transition-colors shadow-sm hover:shadow-md">
                      View Details
                    </button>
                  </div>
                </div>
